      ...
      <div class="preset-chips-section">
        ...
        <div class="preset-cascade-row">
          ...
          <select id="presetModelSelect" class="preset-select" (change)="onPresetSelectChange($event)" [disabled]="!selectedPresetBrand">
            <option value="" disabled selected hidden>Selecciona un modelo</option>
...
export class QuoteFormComponent implements OnInit {
  ...
  public selectedPresetBrand: string = ''; // 👈 Inicialmente vacío

  get filteredPresetVehicles(): VehicleCatalogItem[] {
    // Si no hay marca seleccionada, mostrar todos los vehículos
    if (!this.selectedPresetBrand) {
      return this.presetVehicles;
    }
    // Si hay marca, filtrar solo los de esa marca
  ...
  public applyPresetVehicle(v: VehicleCatalogItem): void {
    ...
    // ✅ Emitir siempre, aunque el formulario no sea válido (pero con datos)
    this.emitQuoteInput();
  }
  ...
  private emitQuoteInput(): void {
    ...
    console.log('📤 Emitiendo datos:', raw);

    const priceNet = Number(raw.priceNet) || 0; // Si es null o NaN, pone 0
    ...
    // ✅ Emitir siempre, incluso si priceNet es 0
