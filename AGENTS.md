...
25. **Ajuste 20 — Anchos fijos de la tabla de Vendedores (26/09/2026):**
    - **Síntoma:** al eliminar la columna Semáforo (Ajuste 18) quedó un hueco vacío entre la celda UBICACION y ACCIONES en pantallas anchas.
    - **Causa:** `.users-table` es `table-layout: auto` con `width: 100%`. Cuando el contenedor es más ancho que el contenido, el sobrante lo reparten **solo** las columnas sin tope. `.asesor-cell` (200px) y `.location-cell` (240px) estaban topadas, así que ACCIONES — la única sin tope — absorbía todo el sobrante; como su contenido está alineado a la derecha, el vacío aparecía justo a su izquierda.
    - **Solución:** `table-layout: fixed` + un `<colgroup>` en `admin-sellers.html` con los 6 anchos: **16% ASESOR · 13% AGENCIA · 17% NOMBRE · 12% WHATSAPP · 25% UBICACION · 17% ACCIONES**. El reparto deja de depender del contenido, así que no hay dónde dejar el sobrante.
    - **Los `max-width` de ASESOR y UBICACION se conservan** a propósito: ambas usan `white-space: nowrap` + `text-overflow: ellipsis` y **el tope es lo que hace funcionar los puntos suspensivos**. Sin tope, una dirección larga se vuelve el ancho mínimo de la celda (no tiene puntos de corte), la tabla ya no puede encogerse y aparece una barra de desplazamiento horizontal permanente.
    - `.users-table td` gana `overflow: hidden` (y `.actions-column` lo revierte a `visible`) para que una marca larga no desborde su celda ahora que el ancho es fijo. En la media query de 768px se revierte a `visible` porque en modo tarjeta cada celda es un contenedor flex.
    - **Móvil:** `.action-buttons` pasó de `grid-template-columns: repeat(3, 1fr)` a `repeat(auto-fit, minmax(44px, 1fr))`. Con el `*ngIf="canManageNotas"` del botón de notas, un socio sin ese permiso solo pintaba 2 botones y la tercera celda del grid quedaba literalmente vacía. Se añadió `margin-right: -0.35rem` a `.action-buttons` para compensar el `gap` que la regla de grid no cuenta como columna.
    - Los porcentajes son un punto de partida, no un valor medido: si algún día se aprieta algo, se ajusta un porcentaje, no el enfoque.
26. **Ajuste 21 — Vistas en celular (26/09/2026):**
    - **Filtros de Seguimiento (bug de flex).** En `admin-seguimiento.css`, la media query de 900px ponía `.toolbar { flex-direction: column }` pero solo cambiaba `min-width` del `.search-wrapper`, dejando vivo el `flex: 1 1 260px` de la base. Al voltear a columna ese `flex-basis` pasa a medir **altura** y el `flex-grow: 1` lo estira más: el buscador quedaba altísimo, la lupa flotando en `top: 50%` de una caja enorme y los selects al fondo. **Regla: al voltear un contenedor flex a `column`, hay que resetear `flex: 0 0 auto` en los hijos** (espejo de lo que ya hace `admin-sellers.css`).
    - **Mis Cotizaciones pasa a lista con scroll.** La media query de 1100px ya no convierte la tabla en tarjetas apiladas (se eliminaron `thead { display: none }`, `tbody/tr/td` como `grid`/`block`, el `::before` con `data-label`, las reglas de `grid-column` y los `tfoot` en `block`). Ahora replica a Admin → Seguimiento: `.lista-wrapper { overflow-x: auto; }` + `.seller-followup-table { table-layout: auto; min-width: 1240px; }`. Se conservan las filas separadas (`border-spacing: 0 8px` + fondo/radio/sombra en el `tr`), los `etapa-chip` de 28px y la leyenda del `<tfoot>` (`<td colspan="14">` normal).
    - `table-layout: auto` a propósito: con `fixed` las 14 columnas (6 datos + 8 etapas + entrega) quedaban del mismo ancho. Arriba de 1100px la tabla **no cambia**: sigue comprimida con `fixed` y sin scroll, como hoy.
    - **`min-width: 1240px` es un punto de partida**, no un valor medido: depende de cuánto larga sea la columna Activo en los datos reales. Si al desplazar queda mucho aire, se ajusta ese número.
    - **Botones de Materiales en celular.** `.dashboard-header { display: none }` a ≤768px se llevaba los tres botones. Se agregó `.mobile-materiales`, oculto en escritorio y con `display: grid` de 2 columnas a ≤768px (el tercero a lo ancho, porque un texto por columna se partiría en tres líneas). Mismo `*ngIf="esVendedor()"` y mismos handlers que la versión de escritorio.
    - **Espacio para la barra inferior.** La barra fija mide `min-height: 60px` + safe area, pero `.dashboard-cotizaciones` solo reservaba `1.5rem` de padding inferior: la última fila quedaba debajo. A ≤640px ahora es `calc(60px + env(safe-area-inset-bottom, 0px) + 1rem)`.
    - **No es testeable con unit tests:** los tres cambios son CSS puro y jsdom no calcula layout. Solo el bloque `.mobile-materiales` tiene test (markup).
    - **Encabezado móvil de Mis Cotizaciones.** `.dashboard-header` ya no se oculta a ≤768px (se veía la pantalla arrancando directo con los botones). Ahora se muestra **compacto**: `min-height: 0`, columna y `padding: 1.25rem`, en vez de los 152px de alto de escritorio. `.header-actions` sí se oculta, porque los tres botones de Materiales ya están replicados en `.mobile-materiales` y "Nueva Cotización" vive en la barra inferior. En la franja 641–768px el título ya no sale con la altura de escritorio. Al limpiar esto se eliminaron del bloque de 640px tres reglas que quedaron muertas: el duplicado de `.dashboard-header`, `.header-actions` y `.btn-primary` (este último solo existe dentro de `.header-actions`).
    - **Fondo del encabezado de la lista (desktop y móvil).** `.seller-followup-table th { background: transparent }` anulaba el `var(--surface-subtle)` de `.seguimiento-tabla th`: era válido cuando cada fila era una tarjeta con radio y sombra propias, pero con la lista con scroll el encabezado quedaba flotando sin ancla. El fondo se movió al `<thead>`, **no** a los `<th>`: la tabla usa `border-collapse: separate` con `border-spacing: 0 8px`, así que en las celdas los 8px de separación las convertirían en 6 bloques sueltos; en el `<thead>` los huecos muestran el mismo fondo y se lee como una franja continua.
