...
| Avisos de retención (purga a los 15 días) como notificación   | **Implementado** | `quote-retention.ts` (helpers de sesión), `mis-cotizaciones.ts`, `admin-quotes.ts`, `admin-seguimiento.ts` |
...
13. **Avisos de retención (15 días) como notificación transitoria (22/09/2026):** Los tres mensajes que explicaban la purga automática dejaron de ser interfaz fija (banner en Mis Cotizaciones, notas en Cotizaciones y Seguimiento) y ahora son **toasts** del `ToastService` existente: se disparan al entrar a cada vista, desaparecen solos (6–7 s) y se muestran **una sola vez por sesión** de pestaña (helpers `shouldShowRetentionNotice` / `markRetentionNoticeShown` + `RETENTION_NOTICE_PREFIX` en `utils/quote-retention.ts`, marca en `sessionStorage`; si el almacenamiento no está disponible, el aviso se repite). Funcionan en los dos hosts de toast ya existentes: `app.html` (vistas de vendedor) y `admin-dashboard.html` (rutas admin) — `App.hideChromeRoute()` oculta el host global en `/admin`, pero el layout admin dibuja el suyo. El chip por tarjeta “Se elimina el dd/mm/aaaa” (`purge-chip`) **se conserva**; en él y en los avisos se cambió el emoji 🗑 por un **icono SVG** de papelera (regla de UI: sin emojis, con iconos). Validado con `npm test -- --watch=false` (**20 archivos, 170 tests**) y `npm run build`.
14. **PDF y vista previa idénticos sin importar dispositivo ni tema (22/09/2026):** Dos síntomas con la misma raíz. (a) **PDF:** la descarga desde móvil salía recortada (solo la Opción 1, columnas cortadas) porque `html2canvas` capturaba el elemento con el layout del viewport del teléfono: `width: element.scrollWidth` se medía en ~360 px y, aunque se pasaba `windowWidth: 1280`, los media queries de `quote-breakdown.css` ya habían compactado el documento. Ahora `PdfExportService.exportToPdf()` delega en `renderDesktopSnapshot()`: clona `#official-pdf-sheet` en un **iframe oculto de 1280 px** (fuera de pantalla, `left: -10000px`), copia ahí los `<style>`/`<link>` de la app, fija el ancho canónico de la hoja (`SHEET_RENDER_WIDTH = 1000`, = `max-width` de `.official-sheet`), neutraliza el `zoom` de ajuste que la vista móvil aplica al clon, espera la carga de imágenes del clon y captura desde ese iframe, que se elimina en `finally`. Además **se eliminó el forzado de tema claro**: ya no se inyecta `ThemeService`, así que el PDF es exactamente igual con tema claro u oscuro (los colores de la hoja son literales). (b) **Vista previa:** la hoja **ya no se re-estiliza en pantallas angostas** — se quitaron del CSS el bloque `@media (max-width: 1024px)`, las reglas de hoja del bloque de 768 px (que ahora solo estiliza la barra de acciones) y todo el bloque de 480 px; en su lugar `QuoteBreakdownComponent` mide su contenedor con `ResizeObserver` y, cuando es más angosto que el ancho canónico, aplica `zoom = ancho / 1000` a la hoja (`[style.zoom]`) y la clase `is-fitted` en `.sheet-scroll-container` (hoja a 1000 px + `overflow-x: hidden`, sin scroll lateral). En escritorio el contenedor es ≥ 1000 px, así que el zoom queda en 1 y **el comportamiento previo no cambia**. Resultado: móvil, tablet y escritorio muestran la misma cotización que exporta el PDF, sin recortes ni estilos compactos. Cubierto por `src/app/services/pdf-export.service.spec.ts` (4 tests) y `src/app/components/quote-breakdown/quote-breakdown.spec.ts` (3 tests). Validado con `npm test -- --watch=false` (**22 archivos, 177 tests**) y `npm run build`.
